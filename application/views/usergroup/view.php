...
<body>
    ...
    var arr = ['ID', 'USER_GROUP_NAME', 'CREATED_DATE', 'CREATED_BY', 'STATUS', 'STATUS'];
        ...
        columnDefs: [{
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    ...
                    else html += "<span class='badge bg-info'>" + data[4] + "</span>";
                    return html;
                },
            },
            {
                ...
                render: function(data, type, row, meta) {
                    ...
                    htmledit =
                        ...
                        data[0] +
                        '\')" type="button" class="btn light btn-success btn-sm sharp"><i class="fas fa-pen"></i></button>' +
                        '</div>';

                    htmlstatus =
                        '<div style="float: left; margin-right: 2px;">' +
                        '<button data-toggle="tooltip" data-placement="top" title="' +
                        (data[4] == 1 ? "Deactivate" : "Activate") +
                        '" onclick="' +
                        (data[4] == 1 ? "inactive" : "active") +
                        "('" +
                        data[0] +
                        '\')" type="button" class="btn btn-sm sharp mr-1 btn-' +
                        (data[4] == 1 ? "danger" : "primary") +
    ...
    function changeSelectStatus(dis) {
        if (dis.value != "" && dis.value != null) {
            t.column(5).search(dis.value).draw();
        } else {
            t.column(5).search(dis.value).draw();
        }
    }
    ...
    function openEdit(id) {
        $.ajax({
            url: url + "User_group/get_edit/" + id,
            dataType: "JSON",
            type: 'GET',
            success: function(data) {
                $("#UgroupNameEdit").val(data.USER_GROUP_NAME);
                $("#IDUserGroupEdit").val(id);
                $('#modalUserGroupEdit').modal('show');
            },
            error: function(jqXHR, textStatus, errorThrown) {
    ...
    function inactive(id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url('user_group/inactive/'); ?>",
                    dataType: "JSON",
                    data: {
                        id: id,
                        tbl: "MS_USER_GROUP",
    ...
    function active(id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url('user_group/active/'); ?>",
                    dataType: "JSON",
                    data: {
                        id: id,
                        tbl: "MS_USER_GROUP",
