...
class Api extends CI_Controller
{
    ...
    public function forgot_password()
    {
        ...
        $user = $this->db->get_where('MS_USERS', ['EMAIL' => $email, 'STATUS' => 1])->row();

        if ($user) {
            $token = base64_encode($this->generateRandomString(32));
            $forgot_password = [
                'ID_USER' => $user->ID,
                'EMAIL' => $email,
                'STATUS' => 1,
                'CREATED_DATE' => date('Y-m-d H:i:s'),
                'IP_LOG' => $this->Global_model->get_client_ip()
            ];
            $user_token = [
                'EMAIL' => $email,
                'TOKEN' => $token,
            ...
            $this->db->insert('FORGET_PASSWORD', $forgot_password);
            ...
            $res['msg'] = "Failed: Email is not registered. Please Try Again";
            $res['status'] = false;
        }
        echo json_encode($res);
    }
    public function resetpassword()
    {
        ...
        if ($user) {
            ...
            if ($user_token) {
                ...
                $res['msg'] = "Failed: Reset password failed! Wrong token.";
                $res['status'] = false;
            }
        } else {
            $res['msg'] = "Failed: Reset password failed! Wrong email.";
            $res['status'] = false;
        }
        echo json_encode($res);
    }
    public function changePassword()
    {
        ...
        if ($user) {
            ...
            if ($user_token) {
                ...
                $res['msg'] = "Password has been changed! Please login.";
                $res['status'] = true;
            } else {
    ...
    public function _sendEmail($token, $email)
    {
